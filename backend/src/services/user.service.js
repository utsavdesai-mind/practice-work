...
exports.createUser = async (data) => {
  ...
  data.role = await resolveEntityByIdOrName(Role, data, "role") || (await Role.findOne({ name: "User" }))._id;
...
exports.getUsers = async (filter = {}) => {
  const adminRoles = await Role.find({ name: { $in: ["Admin", "Super Admin"] } }).distinct("_id");
...
exports.updateUser = async (id, data) => {
  ...
  data.company = await resolveEntityByIdOrName(Company, data, "company") || user.company;
  data.department = await resolveEntityByIdOrName(
    Department, data, "department", { company: data.company }
  ) || user.department;
