...
exports.register = async (data) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      resultUser = user;
      resultToken = generateToken(user._id, role.name);
    });
  } catch (err) {
    throw err;
  } finally {
...
exports.login = async ({ email, password }) => {
  ...
  let role = null;
  if (user.role) role = await Role.findById(user.role);
  if (!role) role = await Role.findOne({ createdBy: user._id });
  if (!role) throw new ApiError(404, "Role not found for user");

  let company = null;
  if (user.company) company = await Company.findById(user.company);
  if (!company) company = await Company.findOne({ createdBy: user._id });
